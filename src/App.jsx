...
class ErrorBoundary extends React.Component {
  ...
  render(){ return this.state.hasError? (
    <div className="p-6 text-white bg-red-900/60 rounded-2xl">
...
export default function App(){
  ...
  useEffect(()=>{ const canvas=canvasRef.current; if(!canvas) return; const ctx=canvas.getContext("2d");
    ...
    const bg=ctx.createLinearGradient(0,0,0,ch); bg.addColorStop(0,bgTop); bg.addColorStop(1,bgBottom); ctx.fillStyle=bg; ctx.fillRect(0,0,cw,ch);
    layers.forEach((L)=>{ if(!L.img?.complete) return; ctx.save(); ctx.globalAlpha=L.opacity; const sw=Math.max(1,Math.round((L.img.width||1)*L.scale)); const sh=Math.max(1,Math.round((L.img.height||1)*L.scale)); const tmp=document.createElement("canvas"); tmp.width=sw; tmp.height=sh; const tctx=tmp.getContext("2d"); tctx.filter=`brightness(${L.bright}) contrast(${L.contrast}) saturate(${L.saturate}) hue-rotate(${L.hueRot}deg)`; tctx.translate(L.flipX?sw:0,0); if(L.flipX) tctx.scale(-1,1); tctx.drawImage(L.img,0,0,sw,sh); const soft=featheredCanvasFrom(tmp,sw,sh,Math.max(0,Math.min(400,Math.round(L.feather)))); ctx.translate(L.x,L.y); ctx.rotate(rad(L.rot)); ctx.drawImage(soft,-sw/2,-sh/2); ctx.restore(); if(showGuides && selected===L.id){ ctx.save(); ctx.translate(L.x,L.y); ctx.rotate(rad(L.rot)); ctx.strokeStyle="rgba(255,255,255,0.8)"; ctx.setLineDash([6,6]); ctx.strokeRect(-sw/2,-sh/2,sw,sh); ctx.setLineDash([]); ctx.restore(); } });
